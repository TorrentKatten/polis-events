...
class JsonConvert(object):
    ...
    @classmethod
    def class_mapper(clsself, obj):
        for keys, cls in clsself.mappings.items():
            if keys.issuperset(obj.keys()):  # are all required arguments present?
                return cls(**obj)
        else:
            # Raise exception instead of silently returning None
            raise ValueError('Unable to find a matching class for object: {!s}'.format(obj))
...
class LocationApi(object):
    def __init__(self, name: str = "", gps: str = ""):
        self.name = name
        self.gps = gps


@JsonConvert.register
class PoliceEventApi(object):
    def __init__(self, id: str = "", name: str = "", datetime: datetime = "", summary: str = "", url: str = "",
                 type: str = "", location: LocationApi = None):
        self.id = id
        self.name = name
...
class Api(object):
    ...
    def load_recent_events(cls):
        ...
        j = JsonConvert()
        return j.FromJSON(response.text)


if __name__ == '__main__':

    print(Api.load_recent_events())
